...
SCIFMT = '{:6.2f}'
FILE_TYPE = 'tgr'  # 'tgr' # 'x26'  # file ending to read

# read the headers of MICA processed images
dirs = [os.path.join(MICA_DIRS, f) for f in os.listdir(MICA_DIRS) if f.startswith('Tgr_')]
mf = []
for d in dirs:
...
# scatter
df1 = copy.deepcopy(df.loc[(df['sky6'] > 1) & (df['Itester'] < 1.5)])
x1 = df1['Itester']
y1 = df1['sky6']
[m, b], residuals, _, _, _ = np.polyfit(x1, y1, 1, full=True)
df1['residuals'] = pd.Series.abs(m*x1 + b - y1)
plt.plot(x1, y1, '.', label='Outlier', color='grey', markersize=1.5)
...
for i in np.sort(df2['date'].dt.month.unique()):
    ...
    plt.plot(df3['Itester'], df3['sky6'], '.', markersize=1.5, color='k')
plt.plot(df3['Itester'], df3['sky6'], '.', markersize=1.5, color='k', label='Used')
plt.plot(x, m*x + b, '-', color='grey', markersize=3, label='$I_{mica}$='+SCIFMT.format(m)+'$*\dfrac{Sky-T}{Sun-T}$ + ' +
